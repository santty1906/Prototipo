...
export function UploadForm({ profiles }: { profiles: { id: string; full_name: string }[] }) {
  ...
  async function uploadOne(file: File, index: number) {
    ...
    if (!ticketResponse.ok) throw new Error(ticket.error ?? "No se pudo iniciar la carga.");
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    for (const [index, file] of selected.entries()) {
      try {
        ...
        update(index, {
          status: "error",
          message: cause instanceof Error ? cause.message : "La carga falló.",
        });
      }
    }
      ...
      >
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Hasta {UPLOAD.maxFilesPerBatch} archivos, {UPLOAD.maxBytes / 1024 / 1024} MB cada uno.
          </p>
        </div>
...
const BATCH_LABELS: Record<FileState["status"], { text: string; className: string }> = {
  queued: { text: "En cola", className: "text-slate-500" },
  uploading: { text: "Cargando…", className: "text-blue-600" },
  done: { text: "Cargado", className: "text-emerald-600" },
  error: { text: "Con error", className: "text-red-600" },
};
